...
/* A: */
// 1. 建立一個 {} bucket。
// 2. 將 nums1 這個 array 的值作為 bucket 的 key；出現過的次數作為 value。ex: { '4': 1, '5': 1, '9': 1 }
// 3. 將 num2 裡的值個別拿出來跟 bucket 作比較。若有等於 bucket 中的 key 等於這個值曾出現在 num1。
// 4. 將個值 push 進 result。接著將 bucket 中以這個值為 key 的 value -1，因為這個次數已經被 push 進 result。
